...
// === HUDSON BOOKING REPORT PROCESSING ===

// Hawaii airport name → IATA code mapping (uses full airport names to avoid substring false positives)
...
// Detect airport IATA code from location text using full airport name matching
function detectAirportFromLocation(location: string): string {
  if (!location?.trim()) return '';
  const normalized = location.trim().toLowerCase();
  for (const [airportName, iata] of Object.entries(HAWAII_AIRPORTS)) {
    if (normalized.includes(airportName)) {
      return iata;
    }
  }
  return '';
...
// Clean airline name from Hudson format: "Lobby 2 Hawaiian Domestic" → "Hawaiian"
// Then look up the IATA code
function lookupAirlineIATA(rawAirline: string): string {
  if (!rawAirline?.trim()) return '';
  let cleaned = rawAirline.trim();

  // Strip "Lobby N" prefix (e.g., "Lobby 2 Hawaiian Domestic")
  cleaned = cleaned.replace(/^lobby\s+\d+\s+/i, '');
  // Strip Domestic/International suffix
  cleaned = cleaned.replace(/\s+(domestic|international)$/i, '');
  ...
  // Exact match
  if (AIRLINE_IATA[normalized]) return AIRLINE_IATA[normalized];

  // Check if cleaned name is contained in any airline name, or vice versa
  for (const [airlineName, iata] of Object.entries(AIRLINE_IATA)) {
    if (airlineName.includes(normalized) || normalized.includes(airlineName)) {
      return iata;
    }
  }
...
// Title-case a string: "YUKO SUZUKI" → "Yuko Suzuki", "mayumi uno" → "Mayumi Uno"
function titleCase(str: string): string {
  if (!str) return '';
  return str.toLowerCase().replace(/\b\w/g, c => c.toUpperCase());
}

// Parse Hudson booking report name formats:
// - "2. YUKO SUZUKI" (numbered prefix)
// - "29. HIROMI INOUE(AULANI)" (numbered + parenthetical)
// - "Coval, Sandra" (last, first)
// - "David Warren" (first last)
// - "WHITE" (single name)
// - "Eugene(ADA)" (parenthetical note)
export function parseHudsonBookingName(rawName: string): { firstName: string; lastName: string } {
  if (!rawName?.trim()) return { firstName: '', lastName: '' };

  let name = rawName.trim();

  // Strip numbered prefix: "2. YUKO SUZUKI" → "YUKO SUZUKI"
  name = name.replace(/^\d+\.\s*/, '');

  // Strip parenthetical: "HIROMI INOUE(AULANI)" → "HIROMI INOUE"
  name = name.replace(/\s*\([^)]*\)\s*$/, '').trim();
  ...
  if (name.includes(',')) {
    ...
    // "First Last" format — use existing splitFullName
  ...
  // Single-name fallback: use as both first and last
...
// Parse Hudson booking date: "2/11/26  04:15 AM" → "2026-02-11"
export function parseHudsonBookingDate(rawDate: string): string {
  if (!rawDate?.trim()) return '';

  // Extract date part before time: "2/11/26  04:15 AM" → "2/11/26"
  const dateMatch = rawDate.trim().match(/^(\d{1,2})\/(\d{1,2})\/(\d{2,4})/);
  if (!dateMatch) return '';
  ...
  if (year.length === 2) {
    ...
    year = (yearNum >= 0 && yearNum <= 50 ? 2000 + yearNum : 1900 + yearNum).toString();
  }

  return `${year}-${month}-${day}`;
}

// Process a Hudson booking report CSV into grouped output
export function processHudsonBookingReport(
  headers: string[],
  data: string[][],
  ...
  const emailIdx = colIndex('EmailAddr');
  const pickupDateIdx = colIndex('Pickup Date');
  const pickupLocIdx = colIndex('PickupLocation');
  const dropoffLocIdx = colIndex('DropOffLocation');
  const directionIdx = colIndex('Direction');
  const airlineIdx = colIndex('Airline');
  const flightIdx = colIndex('Flt. #');
  const paxIdx = colIndex('PAX');
  const phoneIdx = colIndex('Telephone2');
  ...
  for (const row of data) {
    ...
    const { firstName, lastName } = parseHudsonBookingName(rawName);

    // Skip rows with no usable name
    if (!firstName && !lastName) continue;

    const email = getVal(emailIdx);
    const travelDate = parseHudsonBookingDate(getVal(pickupDateIdx));
    const pickupLocation = getVal(pickupLocIdx);
    const dropoffLocation = getVal(dropoffLocIdx);
    const direction = getVal(directionIdx);
    const rawAirline = getVal(airlineIdx);
    const flightNumber = getVal(flightIdx);
    const pax = getVal(paxIdx) || '1';
    const rawPhone = getVal(phoneIdx);

    // Detect airport from locations
    const pickupAirport = detectAirportFromLocation(pickupLocation);
    const dropoffAirport = detectAirportFromLocation(dropoffLocation);

    // Look up airline IATA
    const airlineIATA = lookupAirlineIATA(rawAirline);
    ...
    const outputRow: Record<string, string> = {
      ...
      'Dropoff Airport IATA': '',
      'Dropoff Airline IATA': '',
    ...
    // Place airport/airline/flight in correct columns based on direction
    let detectedAirport = '';
    const isDeparture = direction.toLowerCase() === 'departures';
    const isArrival = direction.toLowerCase() === 'arrivals';

    if (isDeparture) {
      // Departures: hotel → airport, airport is dropoff
      detectedAirport = dropoffAirport;
      outputRow['Dropoff Airport IATA'] = detectedAirport;
      outputRow['Dropoff Airline IATA'] = airlineIATA;
      outputRow['Dropoff Flight Number'] = flightNumber;
    } else if (isArrival) {
      // Arrivals: airport → hotel, airport is pickup
      detectedAirport = pickupAirport;
      outputRow['Pickup Airport IATA'] = detectedAirport;
      outputRow['Pickup Airline IATA'] = airlineIATA;
      outputRow['Pickup Flight Number'] = flightNumber;
    }

    // Determine group key
    const groupKey = detectedAirport
      ? `${detectedAirport}_${isDeparture ? 'Departures' : isArrival ? 'Arrivals' : 'Other'}`
      : 'Other';

    if (!groups[groupKey]) groups[groupKey] = [];
    groups[groupKey].push(outputRow);
  }
